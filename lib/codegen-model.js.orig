 ...
 */
var createVar = function(varName, property,nameSpace) {
	var obj = _.merge({
	...
	if (obj.hasOwnProperty('$ref')) {
		obj.type=typeCamelCase(nameSpace+obj.$ref);
	}
 ...
 * @param {string} classNameSpace nameSpace to prefix objects refs & enums
 * @return {object}            model properties and any enum specification
 */
var createModelProperties = function(swaggerModelProperties, className, nameSpace) {
	/**
	...
	propertyList.forEach(function(rawPropertyName) {
		...
		var varEntry = createVar(property, swaggerModelProperties[property],nameSpace);
		...
		if (varEntry.hasOwnProperty('allowableValues')) {
			...
			var allowedEnumVar = generateEnumDataType(nameSpace + className + '-' + typeCamelCase(property),
				varEntry.description,
				varEntry.allowableValues.enum);

			varEntry.type = nameSpace + className + allowedEnumVar.type;
			...
			if (varEntry.hasOwnProperty('enum')) {
				var enumVar = generateEnumDataType(nameSpace + className + '-' + property, varEntry.description, varEntry.enum);
				...
				if (isArray(varEntry)) {
					...
					if (varEntry.hasOwnProperty('$ref')) { // swagger 2.0 spec
						varEntry.type = typeCamelCase(nameSpace + varEntry.$ref);
					} else {
						varEntry.type = varEntry.hasOwnProperty('$ref') ?
							typeCamelCase(nameSpace + varEntry.$ref) :
							typeCamelCase(nameSpace+varEntry.type);
					}
 ...
 * @param {string} defaultNameSpace default name space value
 * @return {{fileName:string,code:string}[]}  where filename is the name of the file the code shoudl be written
 */
var createModels = function(swagger, templateModel, templateEnum, camelCaseFileName, defaultNameSpace) {
	...
	modelList.forEach(function(model) {
		// determin name space
		var filePrefixNameSpace = model.nameSpace !== undefined ? model.nameSpace + '-' : defaultNameSpace;
		var classNameSpace = typeCamelCase(filePrefixNameSpace);


		// get definition from list 
		var swagegerModelSpec = swaggerModelList[model];

		var nodeModelProperties = createModelProperties(swagegerModelSpec.properties, model, filePrefixNameSpace);

		// adjust for namespace ... 

		var nodeModel = {
			fileName: filePrefixNameSpace + model,
			className: classNameSpace + trimOOInheritance(model),
