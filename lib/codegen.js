...
var getViewForSwagger2 = function(opts, type){
    ...
    // set the models in the template
    _.forEach(swagger.definitions, function(d, model) {
      d.asString = JSON.stringify(d, null, '\t');
      data.models['#/definitions/' + model] = d;
      // runs on action per model if modelAction given
      if(opts.modelAction) opt.modelAction(d, model);
    });
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            var method = {
                ...
                pathName: getPathToMethodName(m, path),
                method: m.toUpperCase(),
                isGET: m.toUpperCase() === 'GET',
                summary: op.description,
                isSecure: op.security !== undefined,
                parameters: [],
                methodAction: opts.methodAction,
                responseMessages: [],
                typeRef: {required:[]}
            };
            ...
            _.forEach(op.responses, function(msg, k){
                msg.code = k;
                var rmodel='';
                if(msg.schema) {
                    msg.type = msg.schema.type;
                    if(msg.schema.items) {
                        rmodel = msg.schema.items['$ref'];
                    }
                    if(msg.schema.properties) {
                        _.forEach(msg.schema.properties, function(prop,k) {
                            method.typeRef.required.push(k);
                        });
                    }
                    else rmodel = msg.schema['$ref'];
                }
                if(rmodel && data.models[rmodel]) {
                    _.forEach(data.models[rmodel].properties, function(prop,k) {
                        method.typeRef.required.push(k);
                    });
                }

                msg.responseModel = rmodel;
                msg.message = msg.description;
                
                method.responseMessages.push(msg);
            });
            

            // puts responseModelRef to the model
            if(method.responseMessages) {
              method.responseMessages.forEach(function(msg) {
                msg.responseModelRef = data.models[msg.responseModel];
              });
            }
...
var getViewForSwagger1 = function(opts, type){
    ...
    // set the models in the template
    _.forEach(swagger.models, function(d, model) {
      d.asString = JSON.stringify(d, null, '\t');
      data.models[model] = d;
      // runs on action per model if modelAction given
      if(opts.modelAction) opt.modelAction(d, model);
    ...
    swagger.apis.forEach(function(api){
        api.operations.forEach(function(op){
            var method = {
                ...
                pathName: getPathToMethodName(op.method, api.path),
                method: op.method,
                isGET: op.method === 'GET',
                summary: op.summary,
                ...
                responseMessages: [],
                type: op.type,
                typeRef: data.models[op.type]
            };
            // set responseMessages
            if(_.isArray(op.responseMessages)) {
                method.responseMessages = op.responseMessages;
            } else if(op.responseMessages) {
                  method.responseMessages.push(op.responseMessages);
            }
            // puts responseModelRef to the model
            if(method.responseMessages) {
              method.responseMessages.forEach(function(msg) {
