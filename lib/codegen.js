...
/**
 * map swagger datatypes to jsDoc data types
 * @param {string} swaggerType type
 * @param [isArrray=false] boolean
 * @returns {string} jsdoc data type
 */
var jsDocType = function(swaggerType, isArrray) {
  var jsDocType = swaggerType;

  switch(swaggerType.toLowerString()) {
    case 'integer' :
      jsDocType = 'number';
      break;
    case 'double' :
    ...
    _.forEach(swagger.paths, function(api, path){
        ...
        _.forEach(api, function(op, m){
            ...
            _.chain(params).forEach(function(parameter) {
                ...
                // find the response returned for
            });
...
var getResponseModelV1 = function(op) {
  var responseModel='object';
  if (op.hasOwnProperty('successMessages')) {
    responseModel = op.successMessages[0].responseModel;
  }
  else { // nothing defined look into responseMessages for response code 200.
    if (_.isArray(op.responseMessages)) {
      // find code ok ...
      op.responseMessages.forEach(function (message) {
        if (message.code === 200) {

          if (message.hasOwnProperty('responseModel')) {
            responseModel = (message.responseModel==='Array' || message.responseModel==='List') ? op.items.$ref+'[]':
