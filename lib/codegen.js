...
var getPathToMethodName = function(path){
    var segments = path.split('/').slice(1).join('-');
    ...
    _.forEach(swagger.paths, function(api, path){
        _.forEach(api, function(op, m){
            var method = {
                path: path,
                className: opts.className,
                methodName: m.toLowerCase() + getPathToMethodName(path),
...
var getCode = function(opts, type) {
    var data = opts.swagger.swagger === 2 ? getView2(opts, type) : getView(opts, type);
