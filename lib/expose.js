    ...
    list.forEach(async (definition) => {
      ...
      if (secure.length > 0) {
        secure.forEach((property) => {
          ...
          validatorsByIdAndType.push(`${origin}['${property.property}'] = await global.FieldValidator.validateByIdAndType('DialRuleSubject','object',${origin}['DialRuleSubject'], this.req, this.res);`);
        });
