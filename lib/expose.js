  ...
  try {
    ...
    list.forEach(async (definition) => {
      ...
      if (secure.length > 0) {
        ...
        secure.forEach((property) => {
          ...
          global.FieldValidator.validate('${property.value}', req.${origin}['${property.definition}']['${property.property}'], req, res).then(function (res) {
            if (!res) return;
          }).catch((err) => { return; });
        `;
        });
    ...
    module.exports = class {
      constructor(req = {}, res = {}, params = {}) {
        ...
        ${parameters}
        ${validation}  
        this.${definition} = ${inspect(definitions[definition], { showHidden: false, depth: null })}; 
      }
    };`;
    ...
    fs.readdir(container, (err, dirs) => {
      ...
      dirs.forEach((dir) => {
        fs.readFile(`${container}/${dir}/${dir}.js`, "utf8", (err, data) => {
          if (err) {
            throw new Error(err.message || err);
          }
