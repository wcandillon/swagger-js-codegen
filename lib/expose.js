  ...
  try {
    ...
    list.forEach(async (definition) => {
      ...
      if (!(definitions[definition].properties instanceof Array)) {
        ...
        let XEntitySystem = 'x-EntitySystem';
        properties.forEach((property) => {
          if (definitions[definition].properties[property]['x-AuthFieldType'] || XEntitySystem) {
        ...
        definitions[definition].properties.forEach((property, i) => {
          if (property['x-AuthFieldType'] || XEntitySystem) {
            methods.forEach((method) => {
              method.parameters.forEach((parameter) => {
                if (parameter.name === definition) {
                  secure.push({
      ...
      if (secure.length > 0) {
        secure.forEach((property) => {
          ...
          validators.push(`${origin}['${property.property}'] = await global.FieldValidator.validate('${props[1]}','${property.parameter_type}',${origin}['${property.property}'], this.req, this.res);`);
