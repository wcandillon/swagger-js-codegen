 ...
 * @param {array} array - array of objects for the comparison
 * @returns {Array}
 */
function removeDuplicates(array) {
  const strings = [];
  array.forEach((object) => strings.push(JSON.stringify(object)));
  const unique = _.uniq(strings);
  const objects = [];
  unique.forEach(string => objects.push(JSON.parse(string)));
  return objects;
}
...
async function split(source, className, path, dir) {
  ...
  try {
    // get the objects before the ES6 conversion
    const objectsListStart = '/* objects >';
    const objectsListStop = '< objects */';
    const objectsDivider = '<----->';
    const lineDivider = '<->';
    const objectsString = source.substring(source.indexOf(objectsListStart) + 12, source.indexOf(objectsListStop));
    const compiledObjects = [];
    const strings = objectsString.split(objectsDivider);
    strings.forEach((string) => {
      const contents = string.split(lineDivider);
      const object = {};
      contents.forEach((line) => {
        const trimmed = line.replace(/\s/g, '');
        if (trimmed !== '') {
          object[trimmed.split(':')[0]] = trimmed.split(':')[1];
        }
      });
      if (Object.keys(object).length > 0) {
        compiledObjects.push(object);
      }
    });
    ...
    methods.forEach((method) => {
      ...
      standalone = standalone.split('await_FieldValidator').join('await FieldValidator');
      destinations.push({
        content: standalone,
        file: destination,
    ...
    files.forEach((file, i) => {
      let bindings = '';
      const uniqueObjects = removeDuplicates(compiledObjects);
      uniqueObjects.forEach((entry) => {
        if (entry.destination === file.file) {
          bindings = `${bindings}
            this.${entry.reference} = new classes.${entry.reference}(req, res, params);`;
        }
      });
      ...
      module.exports = class ${file.file} {
