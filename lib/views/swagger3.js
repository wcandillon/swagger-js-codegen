  ...
  _.forEach(swagger.paths, function(api, path) {
    ...
    _.forEach(api, function(op, m) {
      ...
      if (method.isSecure && method.isSecureApiKey) {
        ...
        data.apiKeyHeaderName = (Object.values(swagger.components.securitySchemes).find((i) => i.type === 'apiKey') || {name: 'X-API-Key'}).name;
      }
      ...
      if (_.isObject(op.requestBody)) {
        ...
        if (headerKeys.length > 0) {
          ...
          console.log(firstHeaderContentType);
          const newParam = {
          ...
          } else if(firstHeaderContentType === 'application/x-www-form-urlencoded'){
            hasBody = false;
          }
