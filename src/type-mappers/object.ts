...
export function makeObjectTypeSpec(swaggerType: SwaggerType, swagger: Swagger): ObjectTypeSpec {
    ...
    const requiredPropertyNames = swaggerType.type === 'object' && isArray(swaggerType.required) ? swaggerType.required : [];
    const properties = concat(getAllOfProperties(swaggerType, swagger), getObjectProperties(swaggerType, swagger, requiredPropertyNames));
...
function getObjectProperties(swaggerType: SwaggerType, swagger: Swagger, requiredPropertyNames: ReadonlyArray<string>): TypeSpec[] {
...
function getAllOfProperties(swaggerType: SwaggerType, swagger: Swagger): TypeSpec[] {
    if (!swaggerType.allOf) {
        return [];
    }

    return flatten(map(swaggerType.allOf, (ref) => {
        if (!ref.$ref) {
        ...
        return flatten(
            filter(
                map(
                    filter(swagger.definitions, (__, definitionName) => definitionName === name),
                    (definition) => {
